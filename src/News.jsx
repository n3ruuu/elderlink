...
const News = () => {
	...
	const activeNews = newsData.filter((news) => news.status === 'Active').sort((a, b) => new Date(a.date) - new Date(b.date))
	...
	useEffect(() => {
		if (!isMobile && !isSinglePageMode) {
			const interval = setInterval(() => {
				setCurrentSlide((prev) => (prev === totalSlides ? 0 : prev + 1))
			}, 3000)
			return () => clearInterval(interval)
		}
	}, [activeNews.length, isSinglePageMode, totalSlides, isMobile])
				...
				<div className="news-list-mobile">
					{activeNews.map((news) => {
						...
						return (
							<div
								key={news.id}
								className="news-item-mobile"
								style={{
									backgroundImage: `url(http://5.181.217.153:5000/uploads/${firstImage})`,
